...
export function ElectionBallotForm() {
  ...
  if (isLoading) {
    return (
      <div className="flex min-h-[50vh] flex-col items-center justify-center space-y-4">
        ...
        <p className="text-sm font-medium text-muted-foreground">Loading your ballot...</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex min-h-[50vh] flex-col items-center justify-center space-y-4 text-center">
        ...
        <div className="space-y-1">
          <h3 className="text-xl font-bold tracking-tight">Unable to Load Ballot</h3>
          <p className="text-sm text-muted-foreground">You may have already voted or the election is inactive.</p>
        </div>
  ...
  const toggleSelection = (positionId: number, maxVotes: number, candidateId: number) => {
    setSelections((prev) => {
    ...
    <div className="mx-auto max-w-4xl animate-in space-y-8 fade-in slide-in-from-bottom-4">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1">
          <Link
            to="/"
            className="hidden md:inline-flex items-center text-sm font-medium text-primary hover:text-primary/80"
          >
      ...
      <div className="space-y-8">
        {ballot.map((position) => {
          ...
          return (
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-border bg-muted/50 px-4 py-3 sm:px-6 sm:py-4">
                <div>
                  <h2 className="text-lg font-semibold">{position.title}</h2>
                  <p className="text-sm text-muted-foreground">
                    Vote for up to {position.maxVotes} candidate{position.maxVotes > 1 ? "s" : ""}
                  </p>
                </div>
                <Badge className="w-fit" variant={remaining === 0 ? "default" : "secondary"}>
                  {remaining === 0 ? "Max Selected" : `${remaining} left`}
                </Badge>
              </div>

              <CardContent className="grid gap-3 p-4 sm:gap-4 sm:p-6 sm:grid-cols-2 md:grid-cols-3">
                {position.candidates.map((candidate) => {
                  const isSelected = selectedForPos.includes(candidate.id);
                  const isDisabled = !isSelected && remaining === 0 && position.maxVotes > 1;

                  return (
                    <button
                      key={candidate.id}
                      disabled={isDisabled}
                      onClick={() => toggleSelection(position.positionId, position.maxVotes, candidate.id)}
                      className={`group relative flex flex-col items-center gap-2 sm:gap-3 rounded-xl border-2 p-3 sm:p-4 text-center transition-all ${
                        isSelected
      ...
      <div className="sticky bottom-2 mx-1 z-10 flex items-center justify-between gap-2 rounded-2xl border bg-background/90 p-3 shadow-lg backdrop-blur-md sm:bottom-4 sm:mx-0 sm:p-4">
        <div className="text-xs font-medium sm:text-sm">
          <span className="text-muted-foreground hidden sm:inline">Total Selections: </span>
        ...
        <div className="flex gap-2 sm:gap-3">
          ...
          <Button
            size="sm"
            className="shadow-md shadow-primary/20 sm:flex hidden"
          ...
          <Button
            size="sm"
            className="shadow-md shadow-primary/20 sm:hidden flex"
            onClick={handleSubmit}
            disabled={totalSelections === 0 || submitVoteMutation.isPending}
          >
